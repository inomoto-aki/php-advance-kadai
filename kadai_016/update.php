...
try{
  ...
  $stmt_select_product = $pdo->book($sql_select_product);

  $stmt_select_product->bindValue(':id',$_GET['id'],PDO::PARAM_INT);

  $stmt_select_product->execute();

  $book = $stmt_select_product->fetch(PDO::FETCH_ASSOC);
  ...
  $genrer_codes = $stmt_select_genre_codes->fetchAll(PDO::FETCH_COLUMN);
   ...
   <form action="create.php" method="post" class="registration-form">
     <div>
       ...
       <select id="genre_code" name="genre_code" required>
         ...
         foreach ($genrer_codes as $gendre_code){
          echo"<option value='{$genrer_code}'>{$genrer_code}</option>";
         }
         ?>
       </select>
     </div>
      <button type="submit" class="sumit-btn" name="submit" value="create">登録</button>
   </form>
  </article>
</main>
<footer>
